...
export default function ArtworkPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const url = `${window.location.origin}/artwork/${id}?view=gallery`
        ...
        <div className="artwork-page" style={{ maxWidth:1100, margin:'0 auto', padding:'32px 20px 60px' }}>
          ...
          <div className="aw-grid">
            ...
            <div style={{ paddingTop:8 }}>
              ...
              {/* Price — gallery/QR view only */}
              {isGalleryView && (artwork.price || artwork.retail_price) && (
          ...
          {artist && siblings.length > 1 && (
            <div className="no-print" style={{ marginTop:32, paddingTop:20, borderTop:'1px solid #e8e3db', display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
              <button
                onClick={() => prevSibling && navigate(`/artwork/${prevSibling.id}${isGalleryView ? '?view=gallery' : ''}`)}
              ...
              <button
                onClick={() => nextSibling && navigate(`/artwork/${nextSibling.id}${isGalleryView ? '?view=gallery' : ''}`)}
