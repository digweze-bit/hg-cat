...
import { supabase, fetchAll } from '../lib/supabasePublic'
...
export default function Catalogue() {
  ...
  useEffect(() => {
    async function load() {
      ...
      supabase.rpc('get_artwork_counts').then(({data:rows, error:rpcErr}) => { if(rpcErr) console.error('RPC error:',rpcErr); if(rows){const c={};rows.forEach(r=>{c[r.artist_id]=Number(r.count)});setWorkCounts(c)} })
      setLoading(false)
    }
    load()
  }, [])
