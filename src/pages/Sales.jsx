...
${client?`<div style="margin-bottom:24px"><div style="font-size:9px;text-transform:uppercase;letter-spacing:.07em;color:#aaa;margin-bottom:5px">Invoice to</div><div style="font-weight:600;font-size:13px;">${e(client.name)}</div>${client.company?`<div style="font-size:12px;color:#6b6760">${e(client.company)}</div>`:""}${client.street||client.address?`<div style="font-size:12px;color:#6b6760">${e(client.street||client.address)}</div>`:""}${client.email?`<div style="font-size:12px;color:#6b6760">${e(client.email)}</div>`:""}${client.phone||client.phone_mobile?`<div style="font-size:12px;color:#6b6760">${e(client.phone||client.phone_mobile)}</div>`:""}</div>`:""}
...
${itemsWithImages.map(it=>`<tr><td style="width:54px;padding:8px 10px 8px 0">${it._imgData?`<img src="${it._imgData}" class="art-img" alt="">`:`<div style="width:46px;height:46px;background:#f0ece7;border-radius:2px;"></div>`}</td><td style="padding:8px 10px"><em style="font-style:italic;font-size:13px;color:#1a1714">${e(it.title)}</em><br><span style="font-size:11px;color:#6b6760">${e(it.artist_name||"")}${it.year?", "+e(it.year):""}</span>${it.medium?`<br><span style="font-size:11px;color:#aaa">${e(it.medium)}${it.dimensions?" &middot; "+e(it.dimensions):""}</span>`:""}</td><td style="text-align:right;white-space:nowrap;padding:8px 0">${formatAmount(it.line_total,inv.currency)}</td></tr>`).join("")}
