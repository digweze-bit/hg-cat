...
// Skip only if the REAL UI is already there
...
// 3. Pass bankAccounts to InvoiceDetail
mustReplace(
  '          userId={user?.id}\n          onClose',
...
mustReplace(
  ...
  '5. Bank state and changeBankAccount'
)

// 6. Add bank account UI before editable notes
mustReplace(
  '            {/* Editable notes */}',
  `            {/* Payment account */}
...
// 7. Update buildInvoiceHTML signature
mustReplace(
...
// 9. Fix the existing broken bank ref in print HTML and add proper bank block before footer
// First remove the old broken bank code that references out-of-scope variables
const oldBrokenBank = `\${(() => {
