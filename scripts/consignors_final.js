...
// 1. Find and add subView state — after modal state
const modalIdx = lines.findIndex(l => l.includes("const [modal, setModal] = useState(null)"))
if (modalIdx < 0) { console.error('modal state not found'); process.exit(1) }
lines.splice(modalIdx + 1, 0, "  const [subView, setSubView] = useState('artworks')")
console.log('OK: 1. subView state')

// 2. Find "Batch upload" button and add sub-nav after its parent div closes
const batchIdx = lines.findIndex(l => l.includes("Batch upload"))
if (batchIdx < 0) { console.error('batch upload not found'); process.exit(1) }
// Find the </div> that closes the page-header area
let headerCloseIdx = batchIdx + 1
while (headerCloseIdx < lines.length && !lines[headerCloseIdx].trim().startsWith('</div>')) headerCloseIdx++
// Insert sub-nav after the closing </div>
const subNav = `
      {/* Sub-navigation */}
...
lines.splice(headerCloseIdx + 1, 0, subNav)
console.log('OK: 2. Sub-nav tabs')

// 3. Find "{/* Filters */}" and add display wrapper before it
const filtersIdx = lines.findIndex(l => l.includes('{/* Filters */}'))
if (filtersIdx < 0) { console.error('Filters comment not found'); process.exit(1) }
lines.splice(filtersIdx, 0, "      <div style={{ display: subView === 'artworks' ? 'block' : 'none' }}>")
console.log('OK: 3a. Open wrapper')

// 4. Find "{/* Add/Edit Modal */}" or "{modal &&" and close wrapper + add views before it
const modalJsxIdx = lines.findIndex((l, i) => i > filtersIdx && (l.includes('{modal &&') || l.includes('{/* Add/Edit Modal */')))
if (modalJsxIdx < 0) { console.error('Modal JSX not found'); process.exit(1) }
// Go back to find the closing </div> of the table area
let insertIdx = modalJsxIdx
// Insert closing div and views
const viewsBlock = `      </div>
...
`
lines.splice(insertIdx, 0, viewsBlock)
console.log('OK: 3b. Close wrapper + add views')

// 5. Add ConsignorsView and ConsignmentReport components before final closing
src = lines.join('\n')

// Find the very last line of code (closing of the last function)
const componentCode = `
// ── CONSIGNORS VIEW ──
...
function ConsignmentReport({ artworks, artists }) {
  ...
  function printReport() {
    ...
    const rows = filtered.map(w => {
      ...
      return '<tr>' +
        '<td style="padding:6px 8px;border-bottom:1px solid #e8e3db;vertical-align:middle">' + (img ? '<img src="'+img+'" style="width:48px;height:48px;object-fit:cover;border-radius:3px" />' : '') + '</td>' +
        '<td style="padding:6px 8px;border-bottom:1px solid #e8e3db;font-size:12px"><b>'+w.title+'</b><br><span style="color:#999">'+artist+'</span></td>' +
        ...
        '<td style="padding:6px 8px;border-bottom:1px solid #e8e3db;font-size:11px;text-align:center;color:'+(w.availability==='Available'?'#2d6a4f':w.availability==='Sold'?'#c0392b':'#b8862a')+'">'+w.availability+'</td></tr>'
    }).join('')

    const html = '<!DOCTYPE html><html><head><title>Consignment Report</title><style>@media print{@page{margin:12mm;size:A4 landscape}body{padding:0}}body{font-family:-apple-system,sans-serif;padding:20px;color:#1a1714}table{width:100%;border-collapse:collapse}th{text-align:left;padding:8px;border-bottom:2px solid #1a1714;font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:#999}</style></head><body>' +
      '<div style="display:flex;justify-content:space-between;margin-bottom:16px"><div><div style="font-size:18px;font-weight:600">Consignment Report</div><div style="font-size:12px;color:#999">Hourglass Gallery \\u00B7 '+new Date().toLocaleDateString('en-GB')+'</div></div><div style="font-size:12px;color:#999">'+filtered.length+' works</div></div>' +
      '<table><thead><tr><th></th><th>Artwork</th><th>Consignor</th><th style="text-align:right">Price</th><th style="text-align:center">Comm.</th><th style="text-align:right">Gallery</th><th style="text-align:right">Owner</th><th style="text-align:center">Status</th></tr></thead><tbody>' + rows + '</tbody></table>' +
    ...
    if (!w) { alert('Allow popups'); setPrinting(false); return }
    w.document.open(); w.document.write(html); w.document.close()
    w.focus(); setTimeout(() => { w.print(); setPrinting(false) }, 600)
  }
            ...
            {filtered.map(w => {
              ...
              return (
                <tr key={w.id} style={{ borderBottom:'1px solid var(--line-soft)' }}>
                  ...
                  <td style={{ padding:'6px 8px', fontSize:12, textAlign:'right' }}>{commRate>0?s+Math.round(price*commRate/100).toLocaleString():'\\u2014'}</td>
                  <td style={{ padding:'6px 8px', fontSize:12, textAlign:'right' }}>{commRate>0?s+Math.round(price*(100-commRate)/100).toLocaleString():s+price.toLocaleString()}</td>
...
// Insert components before the last line
const lastExportIdx = src.lastIndexOf('\n')
src = src.slice(0, lastExportIdx) + componentCode + src.slice(lastExportIdx)
console.log('OK: 4. Components added')
